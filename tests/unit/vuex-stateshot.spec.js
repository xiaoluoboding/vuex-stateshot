...
describe.each([
  ...
  if (flag === 'third') {
    describe(description, () => {
      ...
      it('dispatch a action on global/widget module', async () => {
        expect(vm.lang).toBe('fr')
        vm.setLang('zh')
        expect(vm.lang).toBe('zh')
        await flushPromises()
        // logshot(vm)
      ...
      it('commit a mutation on global/widget module', async () => {
        expect(vm.color).toBe('#000')
        vm.changeColor('#4af')
        expect(vm.color).toBe('#4af')
        await flushPromises()
        expect(vm.hasUndo).toBeTruthy()
      ...
      it('call undo multiple times', async () => {
        await vm.undo()
        await vm.undo()
        const { global: { widget } } = await vm.undo()
        expect(widget.color).toBe('#000')
        expect(widget.lang).toBe('fr')
        expect(vm.hasUndo).toBeFalsy()
        expect(vm.hasRedo).toBeTruthy()
        expect(vm.undoCount).toBe(0)
        expect(vm.redoCount).toBe(3)
      ...
      it('call redo', async () => {
        const { global: { widget } } = await vm.redo()
        expect(widget.color).toBe('#000')
        expect(widget.lang).toBe('zh')
        expect(vm.hasUndo).toBeTruthy()
        expect(vm.hasRedo).toBeTruthy()
        expect(vm.undoCount).toBe(1)
        expect(vm.redoCount).toBe(2)
